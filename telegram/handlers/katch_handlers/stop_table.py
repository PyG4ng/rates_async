...
stop_tables_router = Router()


class Form(StatesGroup):
    stop_tables_form = State()


@stop_tables_router.callback_query(F.data == "stop_table")
async def settings_handler(call: CallbackQuery, state: FSMContext, db: Database):
    text = f'*Запустить*\n\n{await get_all_status(db)}'
    await state.set_state(Form.stop_tables_form)
...
@stop_tables_router.callback_query(Form.stop_tables_form, F.data.in_({'1', '2', '3', '4', '5', 'all'}))
async def start_katch_table(call: CallbackQuery, state: FSMContext, db: Database):
    if call.data in ['1', '2', '3', '4', '5']:
        table = int(call.data)
        db.update_status(table_id=table, new_state='OSTANOVIT')
        text = f'*Остановка стола {table}...*'
        await call.message.edit_text(text=escape_all(text), parse_mode=ParseMode.MARKDOWN_V2)

    if call.data == 'all':
        text = '*Остановка всех столов...*'
        await call.message.edit_text(text=escape_all(text), parse_mode=ParseMode.MARKDOWN_V2)
